...
function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/40 bg-white/85 backdrop-blur-xl">
      <nav className="mx-auto flex h-[4.5rem] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8" aria-label="Hoofdnavigatie">
        <a className="group flex min-w-0 items-center gap-3" href="#top" aria-label="Finstri home">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-ink text-white shadow-soft-xl transition group-hover:bg-ocean">
            <ShieldCheck aria-hidden="true" className="h-5 w-5" />
          </span>
          <span className="text-xl font-bold tracking-normal text-ink">Finstri</span>
        </a>

        <div className="hidden items-center gap-8 lg:flex">
          {navigation.map((item) => (
            <a key={item.href} className="text-sm font-semibold text-slate-600 transition hover:text-ink" href={item.href}>
              {item.label}
            </a>
          ))}
        </div>
