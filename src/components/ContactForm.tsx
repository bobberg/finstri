...
function ContactForm() {
  ...
  const canSubmit = useMemo(
    () => form.firstName.trim() && form.lastName.trim() && form.email.trim() && form.consent,
  ...
  return (
    <Section id="contact" className="bg-cloud" innerClassName="grid gap-10 lg:grid-cols-[0.82fr_1fr] lg:items-start">
      <div className="lg:sticky lg:top-28">
        <p className="text-sm font-bold uppercase text-ocean">Secure contact</p>
        <h2 className="mt-3 text-3xl font-bold leading-tight tracking-normal text-ink sm:text-4xl">Plan een gesprek over de volgende stap in financiele planning.</h2>
        <p className="mt-5 text-lg leading-8 text-slate-600">
          Vertel kort waar je naar zoekt. We nemen zorgvuldig contact op en behandelen je gegevens met dezelfde discipline die we in onze producten leggen.
        </p>
      ...
      <form className="rounded-lg border border-line bg-white p-5 shadow-soft-xl sm:p-7" onSubmit={handleSubmit}>
        ...
        <fieldset className="mt-6">
          <legend className="text-sm font-bold text-ink">Waarmee kunnen we je helpen?</legend>
          <div className="mt-3 grid gap-3">
            {helpOptions.map((option) => (
              <label key={option.value} className="flex cursor-pointer items-start gap-3 rounded-md border border-line bg-cloud p-4 transition hover:border-ocean/35 hover:bg-white">
                <input
        ...
        <label className="mt-6 flex cursor-pointer items-start gap-3 text-sm leading-6 text-slate-600">
          ...
          />
          <span>
            Ik geef Finstri toestemming om mijn persoonlijke data op te slaan en te verwerken volgens de{' '}
            <a className="font-bold text-ocean transition hover:text-ink" href="#privacy">
              privacyverklaring
            </a>
          </span>
        </label>
        ...
        <div className="mt-7 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {isSubmitted ? (
            <p className="flex items-center gap-2 text-sm font-bold text-mint" role="status">
              <CheckCircle2 aria-hidden="true" className="h-4 w-4" />
              Bedankt, we nemen snel contact op.
            </p>
          ) : (
            <p className="text-sm text-slate-500">Verplichte velden zijn voornaam, achternaam, e-mail en toestemming.</p>
          )}
        </div>
      </form>
    </Section>
